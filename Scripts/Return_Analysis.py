...
""" This script is the live analysis of Bitcoin return on Bitfinex, Poloniex, Binance and OKEx """
...
class AnalyseData:
        ...
        for name, obj in self.objs.items():
            for key in self.keys:
                ...
                self.df[name, key] = np.append(
                    self.df[name, key], 
                    np.array([[np.float64(obj.data[key])]]), axis=1
                )
                if self.t > 1:
                    self.fd[name, key] = np.append(
                        self.fd[name, key], 
                        np.array([[np.float64(obj.data[key]) - self.df[name, key][0, self.t-2]]]), 
                        axis=1
                    )
        return self.data

    async def looper(self, ax):
        """ Asynchronous loop to analyse and plot data on the spot """
        last = None
        wait = 0.9
        reg = time.time()
        self.mean_loop = 0
        self.xlim_min, self.xlim_max = 0, 40
        self.ylim_min, self.ylim_max = -30, 30
        self.timer = time.time()
        async for data in self:
            ...
            if not last:
                last = data.copy()
            text = self.text(
                self.fd['Poloniex', 'close'], 
                self.fd['Bitfinex', 'close'], 
                self.fd['Binance', 'close'], 
                self.fd['OKEx', 'close'],
                Min=np.min, 
                Mean=ST.mean, 
                Max=np.max, 
            ...
            for i in range(n):
                ...
                for name in self.names:
                    self.l[i][name], = ax[i].plot(
                        np.arange(self.t),
                        self.fd[name, self.keys[i]].flatten(),
                        self.color[name], 
                        lineWidth=self.lwidth
                    )
                    ax[i].legend(self.names)
                    ax[i].set_title('First difference of prices', fontsize=18, y=1.05)
    ...
    def plot_histo(self, data, ax, txt):
        ...
        for i in range(len(ax)//2):
            if np.var(self.fd[self.names[i], 'close']) > 1 and self.t > 2:
                if not self.h[i]:
                    ax[i].clear()
                    self.h[i] = ax[i].bar(
                        range(h+1), 
                        ET.auto_corr_emp_vect(self.fd[self.names[i], 'close'].T, h=h).reshape(-1), 
                        width=0.9, 
                    ...
                    gama = ET.auto_corr_emp_vect(self.fd[self.names[i], 'close'].T, h=h)
        ...
        for i in range(len(ax)//2, len(ax)):
            minus = np.min(self.fd[self.names[i-4], 'close'])
            maxi = np.max(self.fd[self.names[i-4], 'close'])
            if self.t > 2 and maxi - minus > 1:
                density = ET.kernel_density(self.fd[self.names[i - 4], 'close'].T).flatten()
                grid = np.linspace(minus, maxi, self.t)
                if not self.h[i]:
                    ax[i].clear()
                    self.h[i], = ax[i].plot(grid, density, self.color[self.names[i - 4]], lineWidth=2)
                    if i == 6:
                        ax[i].set_xlabel('First diff USD', x=1, fontsize=14)
    ...
    def width(self):
        ...
        self.lwidth = int(((self.STOP - self.t)/self.STOP)*10)/10+0.1
